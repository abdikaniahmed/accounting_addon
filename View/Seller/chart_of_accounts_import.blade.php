...
<div class="card">
    <div class="card-body">
        <form action="{{ route('seller.accounting.coa.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <a href="{{ route('seller.accounting.coa.sample.download') }}" class="btn btn-link">
                <i class="bx bx-download"></i> {{ __('Download Sample Excel') }}
            </a>

            <a href="{{ route('seller.accounting.coa.index') }}" class="btn btn-outline-secondary ml-2">
