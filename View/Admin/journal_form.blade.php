...
<section class="section">
    ...
    <form action="{{ isset($entry) ? route('admin.accounting.journals.update', $entry->id) : route('admin.accounting.journals.store') }}" method="POST">
        @csrf
        @if(isset($entry))
            @method('PUT')
        @endif
        <div class="card">
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="date" name="date" class="form-control"
                               value="{{ old('date', isset($entry) ? $entry->date->format('Y-m-d') : '') }}"
                               required>
                    </div>
                    <div class="form-group col-md-4">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control" rows="3">{{ old('description', $entry->description ?? '') }}</textarea>
                </div>
                ...
                <div id="account-rows">
                    @php
                        $rows = old('accounts', isset($entry) ? $entry->items : [['account_id' => '', 'debit' => '', 'credit' => '', 'description' => '']]);
                    @endphp
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-primary mt-3">{{ isset($entry) ? __('Update Journal') : __('Save Journal') }}</button>
                    <a href="{{ route('admin.accounting.journals') }}" class="btn btn-light mt-3">{{ __('Cancel') }}</a>
                </div>
            </div>
        </div>
    </form>
