...
<section class="section">
    ...
    <form method="POST"
        action="{{ isset($entry) ? route('admin.accounting.journals.update', $entry->id) : route('admin.accounting.journals.store') }}">
        @csrf
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="mt-3">
                    ...
                    <a href="{{ route('admin.accounting.journals') }}" class="btn btn-light">
                        {{ __('Cancel') }}
                    </a>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Init Select2 on account selects (works for existing and new rows)
    const initSelect2 = (root) => {
        const nodes = root ? qa(root, '.account-select') : qa(document, '.account-select');
