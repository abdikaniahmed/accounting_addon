...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.accounting.groups.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <a href="{{ route('admin.accounting.groups.sample.download') }}" class="btn btn-link">
                <i class="bx bx-download"></i> {{ __('Download Sample Excel') }}
            </a>

            <a href="{{ route('admin.accounting.groups.index') }}" class="btn btn-outline-secondary ml-2">
